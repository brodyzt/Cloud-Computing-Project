...
def run_query(input_query):
    ...
    with IDisposable(cosmos_client.CosmosClient(HOST, {'masterKey': MASTER_KEY} )) as client:
        try:
            ...
            for item in iter(result_iterable):
                new_row = [item["Rumination_Raw_Data"], item["Weekly_Rumination_Average"], item["Raw_Activity_Data"], item["Daily_activity"]]
...
run_query('SELECT * FROM server s WHERE s.time_received > 1556155078034')
