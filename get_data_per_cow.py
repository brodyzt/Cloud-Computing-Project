#creates a separate CSV for each cow in the dataset.
...
with open('ANSC-CS Data-CalvingPredictionSCR.csv') as csv_file:
    ...
    for key, value in result.items():
        with open('fake_cow_data/' + key + '.csv', mode='w') as csv_file:
            fieldnames = ['ID','LACT','Time_Born','AdjTime','N_CALVES','CSEX','NUM_DEAD',
            'AGEFDAT','CINT','GEST','PDIM','DDRY','PDOPN','PREFR','PTOTF','PTOTM','PTOTS',
            'EASE','Time','Raw_Activity_Data','Activity_Change','Activity_Change_by_2_Hours',
            'Rumination_Raw_Data','Weighted_Rumination_Change','Rumination_Deviation_by_2_Hours',
            'Total_Rumination_Minutes_In_Last','Daily_Rumination','Weekly_Rumination_Average',
            'Group,new_health_index','Daily_activity','parity','Day','Period','CalvingYN',
            'newvar','period_to_calving']
